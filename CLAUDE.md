...
4. **Batch upload path**: `FileUpload.tsx` (batch mode tab) accepts a `.zip` archive, a folder (via `webkitdirectory`), or multiple PDF transcripts → `batchTranscriptProcessor.ts` extracts all `.pdf`s using `jszip` (skipping OS artifacts) and processes each through `parseTranscriptPDF` + `generateReport` + `buildCourseGraph` → rendered in `BatchGraphView.tsx` with printable student sheets (`PrintableStudentGraph.tsx`), 1-click batch landscape printing (`window.print()`), and an interactive single-student graph inspection view.
5. **Single-student high-fidelity print**: Clicking "Print" in `StudentBar.tsx` dynamically generates the prerequisite DAG (`buildCourseGraph`) if not loaded, mounts `PrintableStudentGraph.tsx` in `print:block`, and hides the cockpit with `print:hidden`. Dynamically sets `document.title` to the student's name for clean PDF naming, and suppresses trailing blank pages on single-student prints.
6. **Advisor Guide & Audit Logging**: `Header.tsx` includes an "Advisor Guide" launcher (`AdvisorGuideModal.tsx`) with a 6-tab bilingual onboarding manual. Key actions across upload, department change, report download, batch/single print, and study plan print are logged asynchronously to Supabase via `src/lib/logging/auditLogger.ts`.
7. **Timetable Schedule Finder**: `ScheduleFinderModal.tsx` matches the student's recommended courses (and selected electives) against 46 default semester groups (`default_schedules.json`) or uploaded timetable PDFs (`scheduleParser.ts`). Prioritizes base cohort groups, uses `departmentPlanMapper.ts` to locate off-semester subjects in department plans, detects time slot collisions across AAST periods (1–6) and days (Sa–Th), and allows advisors to swap/add/drop subjects from the available registration pool in real time.


There is no backend/API route (`src/app` has no subfolders) — everything runs in the browser. Supabase is used strictly via client-side REST calls for append-only audit logging and the AI edge function.
...
5. **Outstanding F course prioritization** (`courseAnalyzer.ts`): in `splitAvailableCourses`, eligible courses with an uncompleted failing grade (`F`) are prioritized ahead of all untaken courses (`priorityTier: 0`, `isOverdue: true`), ensuring students on probation (or in general) are always recommended to register courses with an outstanding $F$ to replace 0.0 grade points and clear prerequisite bottlenecks.
...
### Course-code canonicalization & Regex

`canonicalizeCode()` (`constants.ts`) is the single normalization used everywhere two codes are compared — it strips non-alphanumerics, uppercases, and resolves cross-plan equivalences (e.g. `CCS3601` ⇄ `CAI3101` for AI, and `NE364` ⇄ `UNR2303` for Engineering Economy). The transcript parser, `courseAnalyzer.ts`, and `courseGraphBuilder.ts` all key off it, so a course taken under one code counts everywhere the equivalent code appears. When adding a code alias, add it to `COURSE_CODE_EQUIVALENCE`, not to individual call sites.
- **Course code pattern**: `COURSE_CODE_PATTERN` and `COURSE_LINE_PATTERN` in `transcriptParser.ts` use `^[A-Z]{2,4}\d{3,4}$` to recognize 2-letter prefixes and 3-digit course numbers (e.g. `NE364`).
- **2-credit courses**: `isTwoCreditCourse(canonical)` validates the canonicalized code (e.g., `NE364` properly resolves to 2 credits as `UNR2303`).
- **Title normalization**: `normalizeCourseTitle` in `courseAnalyzer.ts` normalizes "economics" $\rightarrow$ "economy" for title matching in `getCompletedElectives`, `getUngradedElectives`, and `getOutOfPlanCourses`.
...
- **Auto-Save Draft Plans**: Plans and projected grades auto-save to `localStorage` (`planner_plan_${studentID}_${department}`) with a `💾 Plan saved` status badge and can be cleared via "Clear plan".
- **Official Printable Semester Study Plan (`PrintableSemesterPlan.tsx`)**: Clicking "Print Plan" opens a preview modal (`createPortal`) with an official CCIT academic advising study plan and graduation roadmap. Formatted with `@page { size: portrait; margin: 10mm; }` (`body.printing-semester-plan`), student profile, probation status badges, graduation readiness alert, future term grids, academic disclaimer, and Student & Advisor signature sign-off lines. Temporarily sets `document.title` to `"{Student Name} - Semester Study Plan"` for PDF export.
